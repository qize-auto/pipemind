...
def _invent(goal: str) -> str:
    try:
        import pipemind_creative as cr
        plan = cr.invent_solution(goal)
        lines = [f"🎯 目标: {plan['goal']}"]
        for s in plan['分解步骤']:
            lines.append(f"  Step {s['step']}: {s['action']} — {s['detail']}")
        lines.append(f"\n需要新工具: {'是 ✅' if plan['需要新工具'] else '否'}")
        return "\n".join(lines)
    except Exception as e:
        return f"Creative: {e}"

def _invention_report() -> str:
    try:
        import pipemind_creative as cr
...
def _can_handle(task: str) -> str:
    try:
        ...
        tools = [s["function"]["name"] for s in t.get_all_schemas()]
        import pipemind_creative as cr
        r = cr.can_handle(task, tools)
        if r["can_handle"]:
            return f"✅ 当前能力可处理\n匹配工具: {', '.join(r['matched_tools'])}"
        gap_str = "\n".join(f"  ❌ {g}" for g in r["gaps"])
        return f"⚠ 能力不足\n匹配工具: {', '.join(r['matched_tools'])}\n缺口:\n{gap_str}"
    except Exception as e:
        return f"Error: {e}"

def _decompose(problem: str) -> str:
    try:
        import pipemind_creative as cr
        steps = cr.decompose(problem)
        return "\n".join([f"  Step {s['step']}: {s['action']} — {s['detail']}" for s in steps])
    except Exception as e:
        return f"Error: {e}"

register("invent_solution", _invent, "针对目标设计创造方案",
    {"type":"object","properties":{"goal":{"type":"string","description":"要实现的目标"}},"required":["goal"]})
register("invention_report", _invention_report, "查看创造记录",
    {"type":"object","properties":{}})
register("absorption_report", _absorption_report, "查看吸收的知识",
    {"type":"object","properties":{}})
register("can_handle", _can_handle, "检查当前能力是否能处理某个任务",
    {"type":"object","properties":{"task":{"type":"string","description":"任务描述"}},"required":["task"]})
register("decompose", _decompose, "将复杂问题分解为可执行的子任务",
    {"type":"object","properties":{"problem":{"type":"string","description":"问题描述"}},"required":["problem"]})

def _imitation_cycle(topic: str, source: str = "") -> str:
    try:
        import pipemind_creative as cr
        cycle = cr.imitation_cycle(topic, source)
        return cr.stage_report(cycle)
    except Exception as e:
        return f"Error: {e}"

def _advance_stage(topic: str, notes: str = "") -> str:
    try:
        import pipemind_creative as cr
        # 简化的推进：记录创造
        cr.record_invention(topic, "模仿创新", notes, notes)
        return f"✅ 已记录 '{topic}' 的进化成果"
    except Exception as e:
        return f"Error: {e}"

register("imitation_cycle", _imitation_cycle, "启动模仿→超越进化循环",
    {"type":"object","properties":{
        "topic":{"type":"string","description":"学习主题"},
        "source":{"type":"string","description":"学习来源","default":""}
    },"required":["topic"]})
register("advance_evolution", _advance_stage, "记录进化成果，推进到下一阶段",
    {"type":"object","properties":{
        "topic":{"type":"string","description":"进化主题"},
        "notes":{"type":"string","description":"进化成果描述"}
    },"required":["topic","notes"]})

# ═══════════════════════════════════════
#  记忆系统 v2（Claude-Mem 启发）
# ═══════════════════════════════════════

def _mem_v2_context_search(query: str) -> str:
    try:
...
register("mem_worker_status", _mem_v2_worker, "查看记忆 Worker 状态",
    {"type":"object","properties":{}})
register("mem_hooks", _mem_v2_hooks, "查看生命周期钩子状态",
    {"type":"object","properties":{}})
...
def _brain_think(goal: str, steps: str) -> str:
    try:
        ...
        r = b._metacog.think_before_act(goal, plan)
        lines = [f"🎯 {r['goal']}", f"自信度: {r['confidence']}"]
        for risk in r['risks']:
            lines.append(f"⚠ {risk}")
...
register("brain_insights", _brain_insights, "查看近期思考洞察",
    {"type":"object","properties":{}})
register("brain_skills", _brain_skills, "查看技能系统注册的命令",
    {"type":"object","properties":{}})
register("brain_think", _brain_think, "行动前思考评估方案",
    {"type":"object","properties":{
        "goal":{"type":"string","description":"目标"},
        "steps":{"type":"string","description":"步骤（逗号分隔）"}
    },"required":["goal","steps"]})
